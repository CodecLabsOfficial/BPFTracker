...
namespace CodecLabs.BPFTracking.Plugins.Helper
{
    internal static class SDKHelper
    {
        ...
        internal static OrganizationRequest BuildCallAction(IOrganizationService service, string actionName, Dictionary<string, object> actionParams)
        {
            OrganizationRequest req = new OrganizationRequest(actionName);
        ...
        internal static bool AttributeExists(AttributeCollection attributes, string attributeName)
        {
            ...
            foreach (var item in attributes)
            {
                if (item.Key.Equals(attributeName) && item.Value != null)
                {
                    return true;
                }
            }
